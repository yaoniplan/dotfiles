...
URL="$@"
destinationDir="/mnt/yaoniplan/chinaTelecom/$(date +%Y/%m/%d)"
# Create the directory if it does not exist
mkdir --parents "$destinationDir"
...
if [[ -z "$URL" ]]; then
    ...
    yt-dlp --output "%(title)s.%(ext)s" --max-downloads 1 "https://www.ximalaya.com/album/31923706" && mv *.m4a "$destinationDir"/
...
if [[ $URL == *"github.com"* ]]; then
    ...
    wget --output-document="$destinationDir"/"$fileName".zip "$URL"/archive/master.zip
    unsetProxy
    ...
    if [[ $? -eq 0 ]]; then
        ...
        if command -v 7z &>/dev/null; then
            ...
            yes Y | 7z x "$destinationDir"/"$fileName".zip -o"$destinationDir"/
        else
            unzip "$destinationDir"/"$fileName".zip -d "$destinationDir"/
        fi
        echo "Extraction completed!"
        echo "Deleting the file "$fileName".zip..."
        rm "$destinationDir"/"$fileName".zip
