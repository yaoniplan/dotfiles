// ==UserScript==
// @name         Comic Chapter Navigator
// @namespace    https://github.com/yaoniplan/dotfiles/tree/master/.local/src/userscript
// @version      1.0.1
// @description  Press Enter to jump to the next comic chapter on supported comic/manhua sites
// @icon         data:image/svg+xml;base64,
// @author       yaoniplan
// @license      MIT
// @match        https://www.twbzmg.com/*
// @match        https://www.manhuagui.com/*
// @grant        none
// ==/UserScript==

(() => {
  'use strict';

  /*
   * HOW TO ADD A NEW SITE / 如何添加新网站：
   * 1. 浏览器 Ctrl-Shift-c (DevTools) 选中“下一章”按钮。
   * 2. 右键点击该元素 -> Copy (复制) -> Copy outerHTML (复制 outerHTML)。
   * 3. 将其粘贴给 AI 并附带提示词语：“请为这个外层HTML写一个健壮的 CSS Selector，用于定位该按钮。”
   *    (Note: 尽量提供 outerHTML，而不是直接 copy selector，这样 AI 能根据 onclick 等属性写出更稳定的规则)
   */
  const SITE_SELECTORS = {
    'www.twbzmg.com': '#next-chapter',
    'www.manhuagui.com': 'a.btn-red.next[onclick*="nextC"]',
  };

  const isEditable = (target) => {
    if (!target) return false;
    const tagName = target.tagName.toLowerCase();
    return (
      tagName === 'input' ||
      tagName === 'textarea' ||
      tagName === 'select' ||
      target.isContentEditable
    );
  };

  document.addEventListener('keydown', (event) => {
    if (event.key !== 'Enter' || isEditable(event.target)) return;

    const selector = SITE_SELECTORS[window.location.hostname];
    if (!selector) return;

    const nextBtn = document.querySelector(selector);
    if (nextBtn) {
      nextBtn.click();
    }
  });
})();
