...
交互式 Provider 测试器 — 对齐统一契约 + 可复制诊断报告
  ...
  search / get_chapters / resolve_read
...
def open_text_in_viewer(text: str, title: str, suffix: str = ".txt") -> str | None:
    safe = "".join(c if c.isalnum() or c in "-_" else "_" for c in title)[:40]
    fd, path = tempfile.mkstemp(prefix=f"nov_test_{safe}_", suffix=suffix)
...
def test_resolve(prov, chap, card, report: Report):
    ...
    try:
        paragraphs = prov.resolve_read(chap, comic=card)
        if paragraphs:
            print(f"✅ 解析成功，共 {len(paragraphs)} 个段落")
            for i, t in enumerate(paragraphs[:4]):
            ...
            print("⚠️  未获取到正文（空列表）")
        print_structure("resolve_read 返回", paragraphs, preview_limit=3)
        report.add(f"paragraphs count: {len(paragraphs) if paragraphs else 0}")
        report.add_json("paragraphs first 3", (paragraphs or [])[:3])

        if ask_yes("用编辑器打开【正文】？", default=False):
            text = "\n\n".join(paragraphs or [])
            open_text_in_viewer(text, f"content_{name}", ".txt")
        return paragraphs
    except Exception:
        tb = traceback.format_exc()
        report.set_trace(tb)
...
def main():
    ...
    keyword = input("请输入搜索关键字: ").strip() or "重生之医技强国"
    if keyword == "重生之医技强国":
    ...
    sel = fzf_select(card_options, prompt="选择小说")
    if sel is None:
        review_saved_responses()
        print("\n" + report.render())
        return
    card = cards[int(sel.split(":")[0])]
    print(f"\n✅ 选择小说: {card.get('name')} (id: {card.get('id')})")
    print_structure("当前选中的 card", card)
    report.selected["novel"] = {"id": card.get("id"), "name": card.get("name")}
